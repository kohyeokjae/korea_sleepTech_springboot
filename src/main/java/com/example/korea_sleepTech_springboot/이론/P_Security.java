...
/*
    ...
    cf) CSRF(Cross-Site Request Forgery)
        : 자신의 의지와는 상관없이 특정 웹 사이트에 공격자가 원하는 요청을 보내도록 유도하는 웹 보안 취약점
    ...
    2. 인가(Authorization)
    : 인증된 사용자가 특정 리소스에 접근할 수 있는 권한인지 확인
...
// 1. HTTP 요청: 사용자 폼에 ID와 PW를 입력하고 전송
// 2. Authentication Filter 동작: 전달된 ID와 PW의 유효성 검사를 진행
